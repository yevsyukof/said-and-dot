...
func (dbInstance *database) Exec(sql string, args ...interface{}) (int64, error) {
	result, err := dbInstance.conn.Exec(dbInstance.ctx, sql, args...)
	return result.RowsAffected(), err
}

func (dbInstance *database) Query(sql string, args ...interface{}) (Rows, error) {
	rows, err := dbInstance.conn.Query(dbInstance.ctx, sql, args...)
	return newDatabaseRows(rows), err
}

func (dbInstance *database) QueryRow(sql string, args ...interface{}) Row {
	row := dbInstance.conn.QueryRow(dbInstance.ctx, sql, args...)
	return newDatabaseRow(row)
}

func (dbInstance *database) BeginTx() (Transaction, error) {
	tx, err := dbInstance.conn.BeginTx(dbInstance.ctx, pgx.TxOptions{})
	if err != nil {
		return nil, err
	}

	return newDatabaseTransaction(dbInstance.ctx, tx), nil
}
